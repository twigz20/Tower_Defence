...
bool TowerDefence::onTouchBegan(cocos2d::Touch * touch, cocos2d::Event * unused_event)
{
	...
	for (int i = 0; i < starterTurrets.size(); i++) {
		if (starterTurrets[i]->getObject()->getBoundingBox().containsPoint(touchLoc))
		{
			...
			turret->setPosition(starterTurrets[i]->getObject()->getPosition().x, starterTurrets[i]->getObject()->getPosition().y);
			turret->setScene(this);
			selectedTurret = std::move(turret);
...
void TowerDefence::setStartedTurrets()
{
	...
	int counter = 1;
	for (int i = 0; i < starterTurrets.size(); i++) {
		Turret *turret = starterTurrets[i];
		if (turret) {
			turret->setPosition(turretX + xOffset, turretY + yOffset);
			addChild(turret->getObject());
		}
